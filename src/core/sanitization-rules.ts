...
export const DEFAULT_SANITIZATION_RULES: Record<string, SanitizationRule> = {
  ...
  sql: {
    ...
    validate: (value: string) =>
      !/(\b(ALTER|CREATE|DELETE|DROP|EXEC(UTE)?|INSERT|SELECT|UNION|UPDATE)\b)/gi.test(
        value
      ),
  ...
  xss: {
    ...
    validate: (value: string) =>
      !/(javascript:|vbscript:|onload|onerror|onclick|onmouseover)/gi.test(
        value
      ),
...
export function getSecurityViolationType(
  ruleName: string
